...
def add_managers(update : Update , context : CallbackContext):
    ...
    if authantication in auth:
        ...
        split = user_commands.split()
        #add information to temporary database
        username[split[1]] = f"نام نمایندگی :‌ {split[2]} \n شماره همراه : {split[3]} \n وضعیت : {4} \n اگر اطالاعات نشان داده شده اشتباه است یا ایراد دارد حتما با پشتیبانی تماس بگیرید \n @bvbloxic"
        #add to database
        database = sqlite3.connect("info.db")
        curser = database.cursor()
        sql_data = f""" INSERT INTO MANAGERS (username , nameandfamily , phone) VALUES ("{split[1]}" , "{split[2]}" , "{split[3]}")  """
...
def delete_managers(update : Update , context : CallbackContext):
    ...
    if authantication in auth:
        ...
        if splite[1] in username:
            ...
            #message
            update.message.reply_text(f"اطالاعات نمایندگی {splite[1]} با موفقیت حذف گردید")
        else:
